...
export function QuestionScreen({
  ...
  useEffect(() => {
    const keydown = (event: KeyboardEvent) => {
      ...
      if (event.key === "ArrowLeft" || event.key === "ArrowRight") {
        ...
        choose(event.key === "ArrowLeft" ? 0 : 1);
      }
  ...
  useEffect(() => {
    document
      .getElementById(paused ? "pause-resume" : "question-title")
      ?.focus();
  }, [question.id, paused]);
  return (
    <section className="question-screen page-in">
      ...
      <progress
        value={index - 1}
        max={length}
        aria-label={`${t.question} ${index} / ${length}`}
      />
      {paused ? (
          ...
          >
            {order.map((option, i) => (
              <button
                ...
                className={`choice choice-${i} ${selected === i ? "chosen" : ""}`}
                onClick={() => choose(i as 0 | 1)}
              >
                <span className="choice-label">
                  {i === 0 ? "A" : "B"} <span>{i === 0 ? "↙" : "↗"}</span>
                </span>
                <span className="choice-text">
                  {question.options[option].text[locale]}
                </span>
                <span className="choice-bottom">
                  {selected === i ? "✓" : i === 0 ? "←" : "→"}
                </span>
              </button>
            ))}
