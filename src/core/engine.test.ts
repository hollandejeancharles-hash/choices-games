...
const answer = (id: string, option: 0 | 1 = 0, durationMs = 5000): Answer => ({
  questionId: id,
  option,
  durationMs,
});
describe("scoring", () => {
  it("applies timing thresholds and rejects invalid durations", () => {
    expect([2999, 3000, 15000, 15001].map(timingMultiplier)).toEqual([
      1.25, 1, 1, 0.75,
    ]);
    for (const duration of [-1, NaN, Infinity])
      expect(() => timingMultiplier(duration)).toThrow();
  });
  it("keeps timing effects visible even for one answer", () => {
    const bank = [question("a")];
    expect(scoreAnswers(bank, [answer("a", 0, 1000)]).vector.adventure).toBe(
      100,
    );
  ...
  it("detects pair contradictions without changing the score", () => {
    const bank = [question("a"), question("b")].map((q) => ({
      ...q,
      consistency: { pairId: "risk", axis: "adventure" as const },
    }));
    const answers = [answer("a"), answer("b", 1)];
    expect(scoreAnswers(bank, answers).contradictions).toEqual(["adventure"]);
    expect(scoreAnswers(bank, answers).vector).toEqual(
      scoreAnswers([question("a"), question("b")], answers).vector,
    );
  });
  it("does not count unchosen axis weights as evidence", () => {
    const q: Question = {
      ...question("a"),
      options: [
        { text, weights: { adventure: 2 } },
        { text, weights: { reason: 3 } },
      ],
    };
  ...
  it("rejects unknown or repeated answers", () => {
    ...
    expect(() =>
      scoreAnswers([question("a")], [answer("a"), answer("a")]),
    ).toThrow();
...
describe("adaptive selection", () => {
  ...
  it("revisits ambiguous axes and avoids recent themes", () => {
    const bank = [
      ...
      question("b"),
      question("c"),
      question("d", "reason", "work"),
      question("e", "reason", "work"),
    ];
    const history = [answer("a"), answer("b", 1), answer("e")];
    expect(selectQuestion(bank, [history])?.id).toBe("c");
    expect(selectQuestion(bank, [history], ["a", "b"])?.id).toBe("d");
    expect(selectQuestion(bank, [[answer("a"), answer("b", 1)]])?.id).toBe("d");
  });
...
describe("archetypes and compatibility", () => {
  const archetypes: Archetype[] = [-80, 0, 80].map((value, index) => ({
    id: String(index),
    name: text,
    description: text,
    target: vector(value),
  ...
  it("finds closest pairs and divided questions; unanimity has no division", () => {
    ...
    expect(
      compareGroup(bank, players.slice(0, 2)).mostDivisive,
    ).toBeUndefined();
