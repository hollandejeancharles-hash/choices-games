...
function harness({valid=true,configured=true,allowed=true,providerOk=true,type="social"}={}){
 ...
 ()=>({auth:{getUser:async()=>({data:{user:valid?{id:"sender",email_confirmed_at:"today"}:null},error:valid?null:{}})},rpc:async(name,params)=>{calls.push({name,params});return name.startsWith("claim")?{data:{email:"friend@example.test",token:"secret-invite",code:"DUO12345",attempt:"attempt",kind:"friend"},error:allowed?null:{message:"denied"}}:{error:null};}}),
 {env:{get:key=>env[key]},serve:fn=>handler=fn},
 async(url,options)=>{sends.push({url,options});return {ok:providerOk};});
 return {run:()=>handler(new Request("https://function.invalid",{method:"POST",headers:{Authorization:"Bearer user-token","Content-Type":"application/json"},body:JSON.stringify({id:"12345678-1234-1234-1234-123456789abc",type})})),calls,sends};
}
...
const duo=harness({type:"duo"});assert.equal((await duo.run()).status,200);
assert.equal(duo.calls[0].name,"claim_dilemma_duel_email");
assert.match(JSON.parse(duo.sends[0].options.body).text,/\?account=1&duo=DUO12345/);
assert.equal(duo.calls[1].name,"finish_dilemma_duel_email");
